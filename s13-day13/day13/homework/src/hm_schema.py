...
class Host(Base):
    ...
    def __repr__(self):
        return "<Hostobject: id=%s, hostname=%s, ip_addr=%s, port=%s>" % (
        self.id, self.hostname, self.ip_addr, self.port)
...
class UserProfile(Base):
    ...
    hostusers = relationship('HostUser', secondary=UserProfileToHostUser,
                             backref='user_profiles')  # 多对多关联HostUser表类（注意不是表名），中间表类UserProfileToHostUser（注意不是表名），反向字段为user_profiles
    groups = relationship('Group', secondary=UserProfileToGroup,
                          backref='user_profiles')  # 多对多关联Group表类（注意不是表名），中间表类UserProfileToGroup（注意不是表名），反向字段为user_profiles
...
class HostUser(Base):
    ...
    groups = relationship('Group', secondary=HostUserToGroup,
                          backref='host_users')  # 多对多关联Group表类（注意不是表名），中间表类HostUserToGroup（注意不是表名），反向字段为host_users
...
class AuditLog(Base):
    ...
    def __repr__(self):
        return "<AuditLog object: id=%s, userprofile_id=%s, hostuser_id=%s, cmd=%s>" % (
        self.id, self.userprofile_id, self.hostuser_id, self.cmd)
