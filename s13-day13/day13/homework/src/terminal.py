...
from src import interactive, log_model
...
class Tty(object):
    ...
    def run(self):
        ...
        if client:
            ...
            log_model.insert_log(self.user, self.hostuser, u'login', 'login')  # 记录登录日志
            interactive.interactive_shell(self.user, self.hostuser, chan, client)
            log_model.insert_log(self.user, self.hostuser, u'logout', 'logout')  # 记录退出日志
            chan.close()  # 关闭shell
            client.close()  # 关闭ssh通道
            return True
        else:
            commons.err(conf.ERROR_INFO['5001'])
            return False