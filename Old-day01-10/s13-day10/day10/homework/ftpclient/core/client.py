...
--------------------------------------------------------------------------------------------
    ...
    def get(self, user_input):
        ...
        if os.path.isdir(dst_path):  # 判断目标文件是否存在
            ...
            if os.path.isfile(tmp_filename):  # 判断临时文件是否存在（也就是是否需要续传）
                ...
                tmp_file_size = 0
            self.__sk.send(commons.s2b(
                r'get|{"filename":"%s", "sendsize": %s}' % (filename, tmp_file_size)))  # 向服务端发送下载指令接文件名和已近下载的大小
            Confirm = commons.b2s(self.__sk.recv(100)).split('|')  # 获取服务器反馈
            if Confirm[0] == 'ready':  # 如果是ready则准备下载文件
                ...
                print('')
                print('Validating...')
                new_md5 = commons.get_file_md5(tmp_filename)
    ...
    def put(self, user_input):
        ...
        if len(user_input) == 2:
            ...
            if not os.path.isfile(file_name):
                ...
                return None
            self.__sk.sendall(commons.s2b('checkspacesize'))
            res = json.loads(commons.b2s(self.__sk.recv(100)))
            free_size = int(res['freesize']) * 1024 * 1024  # 获取剩余空间大小
            file_size = os.path.getsize(file_name)
            if free_size > file_size:  # 判断剩余空间是否够，够就下载，不够就直接返回
                md5 = commons.get_file_md5(file_name)  # 获取上传文件的md5
                self.__sk.send(commons.s2b(r'put|{"filename":"%s", "file_size":%s, "md5":"%s"}' % (
                os.path.split(file_name)[1], file_size, md5)))  # 发送上传指令，并通知服务端文件名，文件大小和md5
                res = commons.b2s(self.__sk.recv(200)).split('|')  # 获取服务端确认信息
                if res[0] == 'ready':
                    ...
                    print(commons.b2s(self.__sk.recv(200)))
                    print("")
            else:
    ...
    def auth(self, user_input):
        ...
        if len(user_input) == 1:
            ...
            self.__sk.sendall(commons.s2b(
                r'auth|{"username":"%s", "password":"%s"}' % (username, commons.md5(password))))  # 调用服务端的认证方法，验证用户名密码
            res = commons.b2s(self.__sk.recv(200))  # 获取验证结果
    ...
    def cd(self, user_input):
        ...
        """
        if len(user_input) == 2:
            self.__sk.send(commons.s2b('cd|{"path" : "%s"}' % user_input[1]))  # 发送cd指令
            res = json.loads(commons.b2s(self.__sk.recv(200)))  # 获取服务端反馈
            if res['code'] == '500':
    ...
    def __runcmd(self, cmd):
        ...
        :return: 无
        """
        self.__sk.sendall(commons.s2b(cmd))
    ...
    def rm(self, user_input):
        ...
        """
        if len(user_input) == 2:
            self.__sk.send(commons.s2b('rm|{"path" : "%s"}' % user_input[1]))  # 向服务端发送删除指令
            res = json.loads(commons.b2s(self.__sk.recv(200)))  # 获取执行结果（错误码）
            print(self.__code_list[res['code']])
        else:
            print(self.__code_list['401'])
