            ...
            while flag:
                ...
                while True:
                    ...
                    if password == 'r':
                        flag = False
                        break
                    confirm_password = commons.confirm_input(r'^.{6,15}$', '确认密码: ', '输入提示: 确认密码必须与密码一致，输入r返回上级菜单', is_pass=True)
                    if password == 'r':
                        flag = False
                        break
                ...
                mail = commons.confirm_input(r'^[0-9.a-z]{0,26}@[0-9.a-z]{0,20}.[0-9a-z]{0,8}$', '邮箱: ', '输入提示: 邮箱不能为空，输入r返回上级菜单')
                if mail == 'r':
                    flag = False
                    continue
...
''' %( goods['id'],  goods['name'], goods['price'], goods['class'], goods['info'])
    ...
    def shop():
        ...
        while flag:
            ...
            if all_goods:
                ...
                print("商品列表")
                print('-' * 80)
                print(' %s %s %s %s %s' %(commons.strleft('序号', 6), commons.strleft('编号', 7), commons.strleft('商品名', 45), commons.strleft('价格', 10), commons.strleft('分类', 8)))
                for num, goods in enumerate(res_list, 1):
                    print(' %s   %s %s %s %s' %(commons.strright(str(num), 4), commons.strleft(goods['id'], 7), commons.strleft(goods['name'], 45), commons.strleft(str(goods['price']), 10), commons.strleft(goods['class'], 6)))
                print('-' * 80)
    ...
    def show_shopping_cart():
        ...
        if len(cart_list) != 0: # 判断购物车是否为空
            ...
            for cart_item in enumerate(cart_list, 1): #遍历购物车
                ...
                print('%s %s    %s%s    %s%s' %(str(cart_item[0]).center(8),cart_item[1]['id'].center(8), commons.strleft(cart_item[1]['name'],50), str(cart_item[1]['price']).rjust(8), str(cart_item[1]['num']).rjust(8), str(cart_item[1]['subtotal']).rjust(9)))
        else:
    ...
    def change_password():
        ...
        while flag:
            ...
            confirm_password = commons.confirm_input(r'^.{6,15}$', '确认密码: ', '输入提示: 密码长度介于6~15个字符，输入r返回上级菜单', is_pass=True)
