...
""" % (goods['id'], goods['name'], goods['price'], goods['class'], goods['info'])
                    ...
                    while do_flag:
                        ...
                        if do_chose == '1':
                            ...
                            if shopping.get_crurrent_customer():
                                logger.info('用户%s将%s添加到购物车' % (
                                    shopping.get_crurrent_customer()['username'], chose_goods['name']))
                            else:
        ...
        while True:
            ...
            if chose == 'e':  # 判断用户输入
                ...
                if total != 0:  # 判断购物车是否为空
                    ...
                    input('购物车是空的，快去下手吧，按任意键继续')
            elif chose == 'p':
                # 选择p，支付
                if shopping.get_crurrent_customer():
                    if total != 0:
                        res = atm.pay_api('768购物中心', total)
                        if res:
                            shopping.empty_cart()
                        ...
                        input('您的购物车还是空空如也，快去下手吧！，按任意键继续')
                else:
                    input('您还没有登录，请先登录')
            elif chose == 'r':
                # 选择r退出购物车循环，返回上一级菜单
    ...
    while flag:
        print_welcome()
        chose = print_menu(['购物', '注册', '登录', '注销', '修改密码', '查看购物车', '退出'])
        if chose == '1':
            shop()
        elif chose == '2':
            register()
